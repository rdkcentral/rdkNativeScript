...
rtWebSocket::rtWebSocket(const rtObjectRef& options)
//rtWebSocket::rtWebSocket(rtString options)
    : mEmit(new rtEmit()), mWSHub(new uWS::Hub(0, true)), mWs(nullptr),
...
rtError rtWebSocket::connect()
{
  ...
  hub.onMessage(
      [&](uWS::WebSocket <uWS::CLIENT>* ws, char* data, size_t length, uWS::OpCode opCode)
      {
        ...
        e.set("data", message);
        e.set("isBinary", uWS::BINARY==opCode);
