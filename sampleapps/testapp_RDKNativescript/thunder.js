...
var thunderJS = options => {
  ...
  //options.token = "eyJhbGciOiIiLCJ0eXAiOiJKV1QifQ.aHR0cDovL2xvY2FsaG9zdA.Pyr-aoIihlaGwkGhKOVPr5E4oCzqQ4N7d6HIkJOZUHA";
  api = API(options);
...
class Thunder {
  ...
  /**
   * 
   * @param {string} thunder api
   * @param {object} params
   * Function to invoke thunder api
  */
invoke(api, params, callback) {
  var lastDotPosition = api.lastIndexOf(".");
  if (-1 == lastDotPosition) {
    console.log("Invalid service and method name " + api);
    return false;
  }
  var callsign = api.substring(0, lastDotPosition); 
  var method = api.substring(lastDotPosition+1);
  console.log("Invoking " + callsign + ", method - " + method + ", params " + JSON.stringify(params));
  var result = this._thunder.call(callsign, method, params);
  
  result.then(function(response) {
    console.log(method + " invocation result:", response);
    callback(response); // Pass through full response
  }, function(error) {
    console.log(method + " invocation error:", error);
    callback(error); // Pass through raw error
  });
  
  return true;
}

subscribe(callsign, eventName, callbackfn) {
    console.log("subscribing " + callsign + "for event -  " + eventName);
    this._thunder.on(callsign, eventName, function(result) {
        //MADANA
	//parse result and send to callback
        callbackfn(data);
    });
  }
}

function getAccountno(callback)
{
   thunder1 = new Thunder();
   var thunderApi = "org.rdk.System.getRFCConfig";
   var params = {"rfcList": ["Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AccountInfo.AccountID"]};
   thunder1.invoke(thunderApi, params, callback);
}

//appjs code
function getAccountnoCb(response)
{
    var rfcConfig = response["RFCConfig"];    
    if (rfcConfig != undefined)
    {
        var accountId = rfcConfig["Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.AccountInfo.AccountID"];
        if (accountId != undefined)
	{
	}
    }
}
getAccountno(getAccountnoCb);

const contextMap = {};
const videoIds = ["video1", "video2", "video3", "video4", "video5"];
let contextsReady = 0;
let readyToFocus = false;
...
function createAllContexts() {
  videoIds.forEach(videoId => {
    thunder1.invoke("org.rdk.jsruntime.1.createApplication", 
      { options: "player" }, 
      (response) => {
        const contextId = response?.result;
        
        if (contextId) {
          contextMap[videoId] = contextId;
          console.log(`Created ${videoId} context:`, contextId);
          
          if (Object.keys(contextMap).length === videoIds.length) {
            setTimeout(() => {
              const firstVideo = videoIds[0];
              loadVideoFromJS(firstVideo);
              document.querySelector(`[data-video="${firstVideo}"]`)?.focus();
            }, 300);
          }
        } else {
          console.error(`Failed creating ${videoId} context:`, response);
        }
      }
    );
  });
...
function onItemPressed(videoId, url) {
  const id = contextMap[videoId];
  if (id === undefined) {
    console.error(`Context ID not found for ${videoId}`);
    return;
  }

  const params = { id: id, url: url };
  thunder1.invoke("org.rdk.jsruntime.1.runApplication", params, callback);
}

function callback(response) {
  console.log("Thunder API Response:", response);
}

function getApplications() {
  const output = document.getElementById("appOutput");  
  thunder1.invoke("org.rdk.jsruntime.1.getApplications", {}, response => {
  });
}
